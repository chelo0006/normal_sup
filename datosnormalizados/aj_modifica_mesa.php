...
if ($fila = pg_fetch_row($consulta2)){
	//echo 'actualiza';
	$cadena ="UPDATE fecha_mesa_examenes SET fecha='$fecha',fecha_inicio='$fecha_inicio' where idfecha_examenes=$id";
}

$inscribir = dbConsultar($base,$cadena);
if($inscribir) echo 'Examenes actualizado con éxito';

echo '<iframe style="display:none;" onload="window.location.assign(\'abmmesaexamenes.php?mensaje=La modificación se realizo con éxito.\')"></iframe>';
?>